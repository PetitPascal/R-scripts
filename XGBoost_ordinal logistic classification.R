...
test_tmp$impact<-factor(test_tmp$impact,levels=c("mitigating","neutral","promoting","uncertain"),
                        labels=c("mitigating predictor","neutral","promoting predictor","uncertain"))
...
plot1<-ggplot(test_tmp,aes(y=feature,x=mean_val,col=impact))+
  ...
  scale_color_manual("Direction:",
                     na.value="white",
                     values=rev(c("neutral"="darkgrey","promoting predictor"="#C35C33","mitigating predictor"="#40B696","uncertain"="black")))+
...
plot2<-ggplot(test_tmp,aes(y=feature,x=mean_val,fill=impact))+
  ...
  scale_fill_manual("Direction:",
                    na.value="white",
                    values=c("mitigating predictor"="#A6DDCE","promoting predictor"="#F9CBC2","neutral"="white","uncertain"="grey"))+
  theme(strip.text.x=element_text(size=16, colour="black", angle=0),
        strip.background=element_rect(fill="#A6DDCE", colour="black", size=1),
...
for(n_clust in 1:num_class){
  ...
  plot1<-ggplot(test_tmp,aes(y=feature,x=mean_val,col=impact))+
    ...
    scale_color_manual("Direction:",
                       na.value="white",
                       values=rev(c("neutral"="black","promoting predictor"="#C35C33","mitigating predictor"="#40B696","uncertain"="darkgrey")))+
    theme(strip.text.x=element_text(size=16, colour="black", angle=0),
          strip.background=element_rect(fill="#A6DDCE", colour="black", size=1),
          axis.text.y=element_text(size=16,color="black"),
          axis.text.x=element_text(size=16,color="black"),
  ...
  plot2<-ggplot(test_tmp,aes(y=feature,x=mean_val,fill=impact))+
    ...
    scale_fill_manual("Direction:",
                      na.value="white",
                      values=c("promoting predictor"="#F9CBC2","mitigating predictor"="#A6DDCE","neutral"="white","uncertain"="gray"))+
    theme(strip.text.x=element_text(size=16, colour="black", angle=0),
          strip.background=element_rect(fill="#A6DDCE", colour="black", size=1),
          axis.text=element_text(size=16,color="black", face="bold"),
          axis.title=element_text(size=16,face="bold",color="black"),
